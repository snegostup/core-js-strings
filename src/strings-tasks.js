...
function countVowels(str) {
  ...
  let vowelsCount = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.indexOf(lowerCaseString[i]) >= 0) vowelsCount += 1;
  }
  return vowelsCount;
}
...
function isPalindrome(str) {
  const correctString = str.toLocaleLowerCase().replaceAll(/[^a-zа-яё]/gi, '');
  if (correctString === correctString.split('').reverse().join('')) {
    return true;
  }
  return false;
}
...
function extractNameFromTemplate(value) {
  return value.split(' ').slice(1, value.length).join(' ').replace(/!?/gi, '');
}
...
function encodeToRot13(str) {
  const encryption = 'abcdefghijklmnopqrstuvwxyz';
  const constSymbol = ['!', ',', '?', ' '];
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (constSymbol.includes(str[i])) {
      result += str[i];
    } else if (str[i] === str[i].toLowerCase()) {
      result +=
        encryption[(encryption.indexOf(str[i]) + 13) % encryption.length];
    } else {
      result +=
        encryption[
          (encryption.indexOf(str[i].toLowerCase()) + 13) % encryption.length
        ].toUpperCase();
    }
  }
  return result;
